// Core Module
const path = require('path');
const Student = require('./models/student');
// External Module
const express = require('express');
const session = require('express-session')
const mongoDBStore = require('connect-mongodb-session')(session)

const DB_PATH = "mongodb+srv://AttendEase_db_user:nT9Aj7jabesyqTKE@cluster0.kakmv0k.mongodb.net/";
const { isAdmin } = require("./middleware/auth");
//Local Module
const teacherRoutes = require("./routes/teacherRoutes");
const rootDir = require("./utils/pathUtil");
const errorsController = require("./controllers/errors");
const { default: mongoose } = require('mongoose');
const authRouter = require('./routes/authRouter');
const AddRouter = require('./routes/Add_student');
const aboutRouter = require('./routes/aboutRoute');
const markAttendanceRouter = require('./routes/markAttendence');
const AttendenceRouter = require('./routes/attendance');
const studentDashboardRouter = require('./routes/studentDashboard');
const  subjectRoutes=require('./routes/subject');
const app = express();

app.set('view engine', 'ejs');
app.set('views', 'views');

const store = new mongoDBStore({
  uri: DB_PATH,
  collection: 'sessions'
})

app.use(express.urlencoded());

app.use(session({
  secret : "bhagyadeep",
  resave: false,
  saveUninitialized : false,
  store: store
}))

/*app.use((req,res,next)=>{
 req.session.IsLoggedIn = req.session.IsLoggedIn
 next()
})*/
/*app.use((req,res,next)=>{
  if(req.get('Cookie'))
  {
    req.session.IsLoggedIn = req.get('Cookie')?.split('=')[1];
  }
  else
  {
    req.session.IsLoggedIn = false;
  }
  next();
})*/

app.use(express.json()); // important to parse JSON body

app.use(authRouter)
app.use(AddRouter);
app.use(markAttendanceRouter)
app.use(aboutRouter);
app.use(AttendenceRouter);
app.use(studentDashboardRouter);

app.get("/add-subjects", isAdmin, (req, res) => {
  const pageTitle = "Manage Subjects";  // ✅ Page title for head.ejs
  const currentPage = "AddSubjectsPage"; // ✅ Pass current page identifier
  res.render("addSubjects", { 
    IsLoggedIn: true, 
    user: req.session.user, 
    pageTitle,
    currentPage ,// ✅ Pass to EJS,
    toastMessage: null // <--- ADD THIS
  });
});



// isLoggedIn == true ho tabhi next karo nhi toh "/" redirect ho
app.use('/host',(req,res,next)=>{
  if(req.session.IsLoggedIn)
  {
    next();
  }
  else
  {
    res.redirect('/login')
  }
})  // pehle request me isLoggedIn == true ho tabhi next karo nhi toh "/" redirect ho
// app.use("/", attendanceRoutes);
app.get("/mark-attendance", (req, res) => {
  res.render("markAttendance"); // your EJS file name
});

app.use(express.static(path.join(rootDir, 'public')))
app.get("/view", (req, res) => {
  res.render("store/view", { user: req.user });  // Pass user object for nav.ejs
});
app.use('/subject', subjectRoutes);
app.use("/api/teacher", teacherRoutes);
app.use(errorsController.pageNotFound);
// app.get("/view", (req, res) => {
//   res.render("view"); 
// });
// 


const PORT = 3001;

mongoose.connect(DB_PATH).then(() => {
  console.log('Connected to Mongo');
  app.listen(PORT, () => {
    console.log(`Server running on address http://localhost:${PORT}`);
  });
}).catch(err => {
  console.log('Error while connecting to Mongo: ', err);
});

// hamesha trust but verify